 ...
 *
 * Only non-sensitive WordPress object IDs are passed in the redirect URL:
 * - Advisor post ID, when a preferred advisor was selected.
 * - Vacation Type term ID from the booking form.
 *
 * The confirmation page resolves those IDs back to WordPress content and never
 * reads personal traveler information from the query string.
 *
 * @package PrelaunchWP
 */
...
function prelaunch_booking_confirmation_redirect(
	...
	$has_preferred_advisor = trim( (string) rgar( $entry, '8' ) );

	if ( 'Yes' === $has_preferred_advisor ) {
		$advisor_id = absint( rgar( $entry, '9' ) );
	...
	$vacation_type_id = absint( rgar( $entry, '11' ) );
...
function prelaunch_get_booking_confirmation_vacation_type(): ?WP_Term {
	...
	$term = get_term( $term_id, 'vacation_type' );
...
function prelaunch_get_blog_trip_type_for_vacation_type(
	...
	if ( ! $vacation_type instanceof WP_Term || ! taxonomy_exists( 'trip_type' ) ) {
		return null;
	}
	...
	$target_name = $normalize( $vacation_type->name );
...
function prelaunch_get_booking_confirmation_posts(
	...
	$blog_trip_type = prelaunch_get_blog_trip_type_for_vacation_type( $vacation_type );
...
function prelaunch_noindex_booking_confirmation( array $robots ): array {
	if ( is_page_template( PRELAUNCH_BOOKING_CONFIRMATION_TEMPLATE ) ) {
		$robots['noindex'] = true;
