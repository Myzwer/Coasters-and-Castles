<?php
	...
	/**
	 * Randomize the Advisor archive once per calendar day.
	 *
	 * The date becomes the random seed, so:
	 * - Advisor order remains consistent throughout the day.
	 * - Pagination remains stable throughout the day.
	 * - Filtered results use the same daily seed.
	 * - The directory reshuffles automatically the following day.
	 *
	 ...
	 * @return void
	 */
	function prelaunch_daily_advisor_order( WP_Query $query ): void {
		if (
			is_admin()
			|| ! $query->is_main_query()
			|| ! $query->is_post_type_archive( 'advisor' )
		) {
			return;
		}

		$daily_seed = (int) wp_date( 'Ymd' );

		$query->set( 'orderby', "RAND({$daily_seed})" );
		$query->set( 'posts_per_page', 18 );
	}

	add_action(
		'pre_get_posts',
		'prelaunch_daily_advisor_order',
		20
	);
