<?php
	...
	/**
	 * Get a prioritized WP_Query of related posts for a given post.
	 *
	 * Related order:
	 * 1) Same Vacation Type (`trip_type`)
	 * 2) Same Article Type (`article_type`)
	 * 3) Random published posts fallback
	 *
	 * Posts selected in an earlier stage are excluded from later stages so the
	 * final list contains no duplicates. The current post is always excluded.
	 *
	 * @param int|\WP_Post|null $post Post ID or object. Defaults to current post.
	 * @param array $args Optional args.
	 *
	...
	if ( ! function_exists( 'prelaunch_get_related_posts_query' ) ) {
		function prelaunch_get_related_posts_query( $post = null, array $args = [] ): WP_Query {
			...
			/**
			 * Collect related post IDs for a taxonomy.
			 *
			 * @param string $taxonomy Taxonomy name.
			 *
			 * @return void
			 */
			$collect_taxonomy_matches = static function ( string $taxonomy ) use (
				$post,
				$posts_per_page,
				&$selected_ids,
				&$excluded_ids,
				...
				$remaining = $posts_per_page - count( $selected_ids );
				...
				foreach ( $matched_ids as $matched_id ) {
					$matched_id = absint( $matched_id );

					if (
						! $matched_id
						|| in_array( $matched_id, $excluded_ids, true )
					) {
						continue;
					}

					$selected_ids[] = $matched_id;
					$excluded_ids[] = $matched_id;
				}
			};

			// Priority 1: Same Vacation Type.
			$collect_taxonomy_matches( 'trip_type' );

			// Priority 2: Same Article Type.
			$collect_taxonomy_matches( 'article_type' );

			// Priority 3: Random published posts to fill any remaining spaces.
			$remaining = $posts_per_page - count( $selected_ids );
			...
			if ( $remaining > 0 ) {
				...
				foreach ( $fallback_ids as $fallback_id ) {
					...
					if (
						! $fallback_id
						|| in_array( $fallback_id, $excluded_ids, true )
					) {
						continue;
					}

					$selected_ids[] = $fallback_id;
					$excluded_ids[] = $fallback_id;
				}
			}
